---
title: Mencetak email, kalender, dan lampiran dalam Outlook di web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 96cab5ed40813cd5d822ad6c614c93b36733daaa6389df11117c871034c474ba
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: id-ID
ms.lasthandoff: 08/05/2021
ms.locfileid: "54010547"
---
# <a name="print-email-calendars-and-attachments"></a>Mencetak email, kalender, dan lampiran

Untuk mencetak pesan email di Outlook di web:
  
1. Pilih pesan yang ingin Anda cetak.
1. Di sudut kanan atas pesan, pilih tiga titik kemudian pilih **Cetak**.

Untuk mencetak kalender Anda:

1. Di sudut kanan atas halaman, pilih <img src='data:image/png;base64,' />
 **Mencetak**.
1. Pilih rentang waktu dan tampilan, lalu pilih **Cetak.**

Untuk mencetak lampiran:

1. Buka pesan berisi lampiran yang ingin Anda cetak.
2. Pindahkan kursor Anda ke lampiran dan pilih **ikon** Unduh <img src='data:image/png;base64,' />.
3. Setelah file diunduh, buka menggunakan program yang sesuai, lalu pilih opsi untuk mencetak. Misalnya, buka file .doc di Word, lalu cetak.

Untuk mempelajari selengkapnya, [lihat Mencetak pesan email, lampiran, dan kalender Outlook di web](https://support.office.com/article/2cf529d1-3b8f-4de2-b254-b7f870e58a2b).