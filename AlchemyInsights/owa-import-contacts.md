---
title: '8000019: Cara menambahkan atau mengimpor kontak'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: id-ID
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999783"
---
# <a name="import-contacts"></a>Mengimpor kontak

Sebelum mengimpor kontak ke Outlook di web, Anda harus mengekspor kontak dari penyedia email lain sebagai .csv kontak. Lalu, ikuti instruksi di bawah ini.

1. Di bagian bawah halaman, pilih <img src='data:image/png;base64,' />
 **Orang.**
2. Pada toolbar, pilih **Kelola**  >  **Impor kontak**.
3. Pilih **Telusuri**, pilih file CSV Anda, lalu pilih **Buka**.
4. Pilih **Impor.**

Untuk mempelajari selengkapnya, lihat [Mengimpor kontak ke Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

